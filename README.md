...
- Выбор показателя, временного диапазона и сценария
- KPI-карточки: текущее значение, изменение год-к-году, CAGR
- Карточки прогнозов на 2026–2029 с процентным изменением
- Линейный график: фактические данные + прогноз + доверительный интервал
- Горизонтальная SHAP-диаграмма: вклад каждого фактора (зелёный / красный)
...
- Переключатель сценариев, фильтр по категории
- Экспорт в Excel
...
| `DashboardStore` | выбранный показатель, сценарий, диапазон | `timeSeries`, `shapContributions` (вычисляемые) |
...
> **Важно:** на данный момент все данные демонстрационные (хардкод в MobX-сторе). Интеграция с `indicators.db` / `models.db` через API — следующий этап.
