...
Интерактивный дашборд для просмотра прогнозов, анализа факторов и сценарного моделирования.
...
├── widgets/             — шапка, подвал, уведомления, help-виджет
...
|---|---|---|
| `/` | `HomePage` | Лендинг: возможности системы, этапы работы, стек технологий, FAQ |
| `/dashboards` | `DashboardsPage` | Главный аналитический дашборд |
| `/forecasts` | `ForecastsPage` | Сводная таблица прогнозов на 4 года |
| `/scenarios` | `ScenarioModelingPage` | Сценарное моделирование с ползунками факторов |
| `/about` | `AboutPage` | О проекте: цели, методология, научная новизна |
| `/docs` | `DocsPage` | Документация с боковым меню |

### Ключевые страницы

**`DashboardsPage`** — основной рабочий экран:
- Выбор показателя, модели прогноза (ML / LSTM), временного диапазона и сценария
...
- Панель инсайтов: топ-3 позитивных и негативных фактора
...
- Экспорт в Excel
- Таблица всех показателей с прогнозами Y+1…Y+4

### MobX-хранилище (`stores/rootStore.tsx`)

| Store | Данные | Ключевые методы |
|---|---|---|
| `UiStore` | язык интерфейса, уведомления | `setLanguage()` |
| `IndicatorStore` | 48 показателей, избранное, фильтры | `toggleFavorite()`, `setSearchQuery()`, `setActiveCategory()` |
...
> **Важно:** дашборд теперь читает реальные данные через FastAPI (`/api/indicators`, `/api/models`, `/api/dashboard`). Часть страниц всё ещё содержит демонстрационные элементы интерфейса и требует дальнейшей интеграции.
